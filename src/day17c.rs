...
fn solve_part1(matrix:Matrix<i32>){
    ...
    // let goal = Node{position:(8, 0), f_score:0,last_direction:None, dir_count:0};
    ...
    while let Some(current) = open_list.pop(){
        ...
        for (mut successor, direction) in successors{
            // came_from.insert(successor.position, current.position);
            ...
            if open_list.iter().any(|node| node.position == successor.position && node.f_score<temp_f_score){
                continue;
            }

            if close_list.iter().any(|node| node.position == successor.position && node.f_score<temp_f_score){
                continue;
            }
